...
{
    public class
        ExponentialRankSelectorTest : ProbabilitySelectorTesterBase<ExponentialRankSelector<DoubleGene, double>>
    {
        protected override Factory<ExponentialRankSelector<DoubleGene, double>> Factory()
        {
            return () => new ExponentialRankSelector<DoubleGene, double>();
        }
        ...
        public void SelectDistribution(double c, Named<double[]> expected, Optimize opt)
        {
            Retry<Exception>(3, () =>
            {
                const int loops = 50;
                ...
                RandomRegistry.Using(random, r =>
                {
                    var distribution = Distribution(
                        new ExponentialRankSelector<DoubleGene, double>(c),
                        opt,
                        npopulation,
                        loops
                    );
