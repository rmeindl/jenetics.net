...
namespace Jenetics
{
    public class BoltzmannSelectorTest : ProbabilitySelectorTesterBase<BoltzmannSelector<DoubleGene, double>>
    {
        protected override Factory<BoltzmannSelector<DoubleGene, double>> Factory()
        {
            return () => new BoltzmannSelector<DoubleGene, double>();
        }
        ...
        public void SelectDistribution(double b, Named<double[]> expected, Optimize opt)
        {
            Retry<Exception>(3, () =>
            {
                const int loops = 50;
                ...
                RandomRegistry.Using(random, r =>
                {
                    var distribution = Distribution(
                        new BoltzmannSelector<DoubleGene, double>(b),
                        opt,
                        npopulation,
                        loops
                    );
        ...
        {
            const string resource = "Jenetics.resources.BoltzmannSelector";
        ...
        {
            var selector = new BoltzmannSelector<DoubleGene, double>(2);

            // Create population with zero fitness.
            var population = TestUtils.NewDoublePopulation(20, 0, 0);
